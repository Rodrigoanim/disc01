...
def get_element_value(cursor, name_element, element=None):
    """Busca o valor de um elemento na tabela forms_tab_06."""
    cursor.execute("""
...
def process_forms_tab_06(section='inventario_p1'):
    """
    Processa registros da tabela forms_tab_06 e exibe em layout de grade.
    Versão otimizada com controle de st.rerun().
    
    Args:
        section (str): Seção a ser exibida ('inventario_p1', 'inventario_p2' ou 'resultado')
    """
    ...
    try:
        ...
        title_text = titles.get(section, "Assessment 06 - Inventário Vocacional e Profissional")
        st.markdown(f"""
            <p style='
                text-align: left;
                font-size: 28px;
        ...
        # Verifica se estamos no assessment "06" (Inventário Vocacional e Profissional)
        ...
        if assessment_id == "06":
            ...
            main_menu_value = st.session_state.get("inventario_section_selector", None)
            bottom_menu_value = st.session_state.get("inventario_section_selector_bottom", None)
            ...
            radio_params = {
                ...
                "key": "inventario_section_selector_bottom",  # Key diferente para evitar conflito
                "horizontal": True,
            }
            
            # Só usa index se não houver valor no session_state (evita conflito)
            if "inventario_section_selector_bottom" not in st.session_state:
            ...
            def on_bottom_menu_change():
                ...
                selected = st.session_state["inventario_section_selector_bottom"]
                if selected:
                    section_value = section_options[selected]
                    # Atualiza a variável auxiliar que será lida pelo main.py
                    st.session_state["target_section_06"] = section_value
...
def call_insumos(cursor, element):
    ...
    """
    try:
        name = element[0]  # name_element da forms_tab_06
        str_value = element[6]  # str_element da forms_tab_06 (ex: 'InsumosI15')
        
        # Verifica se há uma referência válida
        if not str_value:
            return 0.0
            
        # Busca o math_element na forms_insumos onde name_element = str_value da forms_tab_06
        cursor.execute("""
            SELECT math_element 
            FROM forms_insumos 
            WHERE name_element = ?
...
def process_forms_tab(section='inventario_p1'):
    """
    Função wrapper para compatibilidade com main.py
    Chama process_forms_tab_06 com a seção especificada
    """
    return process_forms_tab_06(section)
