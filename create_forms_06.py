# Arquivo: create_forms_06.py
# Data: 14/12/2025
# Importação: forms_tab_06 e forms_resultados_06 (Inventario Vocacional e Carreira)
...
def select_import_file(expected_filename):
    ...
    title = f"Selecione o arquivo {expected_filename}"
    ...
    # Verificar se o arquivo tem o nome exato esperado
    file_name = os.path.basename(file_path)
    expected_name_lower = expected_filename.lower()
    file_name_lower = file_name.lower()
    
    # Comparação exata (case-insensitive)
    if file_name_lower != expected_name_lower:
        print(f"❌ ERRO: Arquivo incorreto!")
        print(f"   Arquivo esperado: {expected_filename}")
        print(f"   Arquivo selecionado: {file_name}")
        messagebox.showerror("Arquivo Incorreto", 
            f"O arquivo selecionado '{file_name}' não corresponde ao esperado.\n\n"
            f"Arquivo esperado: {expected_filename}\n\n"
            "Por favor, selecione o arquivo correto.")
        return None
    
    print(f"✅ Arquivo válido: {file_name}")
    return file_path
...
def import_forms_tab_06():
    """
    Importa dados do arquivo forms_tab.txt para a tabela forms_tab_06
    """
    if not check_database():
        return False
    
    # 1. Selecionar arquivo TXT
    print("📁 Selecionando arquivo forms_tab_06.txt...")
    txt_file = select_import_file("forms_tab_06.txt")
    if not txt_file:
        return False
    
    # 2. Confirmar seleção
    if not confirm_file_selection(txt_file, "forms_tab_06"):
    ...
    try:
        ...
        print("🔄 Iniciando importação: forms_tab.txt → forms_tab_06")
        
        # 3. Verificar se forms_tab_06 já existe
        cursor.execute("""
            SELECT name FROM sqlite_master 
            WHERE type='table' AND name='forms_tab_06'
        """)
        
        if cursor.fetchone():
            print("⚠️  Tabela forms_tab_06 já existe!")
            root = tk.Tk()
            root.withdraw()
            if not messagebox.askyesno("Confirmação", 
                "A tabela forms_tab_06 já existe. Deseja apagá-la e recriar?"):
                print("Operação cancelada pelo usuário.")
                return False
            
            # Apagar tabela existente
            cursor.execute("DROP TABLE IF EXISTS forms_tab_06")
            conn.commit()
            print("🗑️  Tabela forms_tab_06 removida para recriação.")
        
        # 4. Criar tabela forms_tab_06
        print("📋 Criando tabela forms_tab_06...")
        cursor.execute("""
        ...
        print(f"\n📈 Estatísticas da tabela forms_tab_06:")
...
def import_forms_resultados_06():
    """
    Importa dados do arquivo forms_resultados.txt para a tabela forms_resultados_06
    """
    if not check_database():
        return False
    
    # 1. Selecionar arquivo TXT
    print("📁 Selecionando arquivo forms_resultados_06.txt...")
    txt_file = select_import_file("forms_resultados_06.txt")
    if not txt_file:
        return False
    
    # 2. Confirmar seleção
    if not confirm_file_selection(txt_file, "forms_resultados_06"):
        print("❌ Importação cancelada pelo usuário.")
        return False
    ...
    try:
        ...
        print("🔄 Iniciando importação: forms_resultados.txt → forms_resultados_06")
        
        # 3. Verificar se forms_resultados_06 já existe
        cursor.execute("""
            SELECT name FROM sqlite_master 
            WHERE type='table' AND name='forms_resultados_06'
        """)
        
        if cursor.fetchone():
            print("⚠️  Tabela forms_resultados_06 já existe!")
            root = tk.Tk()
            root.withdraw()
            if not messagebox.askyesno("Confirmação", 
                "A tabela forms_resultados_06 já existe. Deseja apagá-la e recriar?"):
                print("Operação cancelada pelo usuário.")
                return False
            
            # Apagar tabela existente
            cursor.execute("DROP TABLE IF EXISTS forms_resultados_06")
            conn.commit()
            print("🗑️  Tabela forms_resultados_06 removida para recriação.")
        
        # 4. Criar tabela forms_resultados_06
        print("📋 Criando tabela forms_resultados_06...")
        cursor.execute("""
        ...
        # 7. Criar mapeamento de seções baseado na forms_tab_06
        ...
        for index, row in df.iterrows():
            try:
                ...
                # Inserir registro
                cursor.execute("""
                    INSERT INTO forms_resultados_06 (
                        name_element, type_element, math_element, msg_element,
                        value_element, select_element, str_element, e_col, e_row,
                        section, user_id
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        cursor.execute("""
            SELECT 
                COUNT(*) as total_registros,
                COUNT(DISTINCT user_id) as usuarios_unicos
            FROM forms_resultados_06
        """)
        
        stats = cursor.fetchone()
        print(f"\n📈 Estatísticas da tabela forms_resultados_06:")
        print(f"   Total de registros: {stats[0]}")
        print(f"   Usuários únicos: {stats[1]}")
...
def verify_import():
    ...
    try:
        ...
        print(f"📊 forms_tab_06: {'✅ Existe' if forms_tab_exists else '❌ Não encontrada'}")
        print(f"📊 forms_resultados_06: {'✅ Existe' if forms_resultados_exists else '❌ Não encontrada'}")
        
        if forms_tab_exists:
            cursor.execute("SELECT COUNT(*) FROM forms_tab_06")
            count_tab = cursor.fetchone()[0]
            print(f"   Registros: {count_tab}")
        
        if forms_resultados_exists:
            cursor.execute("SELECT COUNT(*) FROM forms_resultados_06")
...
def show_table_statistics():
    ...
    try:
        ...
        print("\n📈 ESTATÍSTICAS DAS TABELAS INVENTARIO VOCACIONAL E CARREIRA:")
        print("-" * 60)
        ...
        if cursor.fetchone():
            ...
            print("\n📊 forms_resultados_06: ❌ Não encontrada")
...
def main():
    ...
    print("=" * 60)
    print("📥 IMPORTAÇÃO: INVENTARIO VOCACIONAL E CARREIRA (forms_tab_06 + forms_resultados_06)")
    print("=" * 60)
    
    while True:
        print("\n📋 MENU DE OPÇÕES:")
        print("1 - Importar forms_tab.txt → forms_tab_06")
        print("2 - Importar forms_resultados.txt → forms_resultados_06")
